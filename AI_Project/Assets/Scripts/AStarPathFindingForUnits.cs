...
public class AStarPathFindingForUnits : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (targetPointIndex < pathRecords.Count && targetPointIndex >= 0)
        {
            ...
            if (Vector3.Distance(transform.position, new Vector3(pathRecords[targetPointIndex].x, pathRecords[targetPointIndex].y, pathRecords[targetPointIndex].z)) < 0.01f && targetPointIndex < pathRecords.Count)
            {
                ...
                {
                    targetPointIndex = -1;
                    if(GameManager.instance.state == GameManager.State.Moving)
                    {
                        GameManager.instance.state = GameManager.State.Idle;
                        GameManager.instance.targetPrefab.SetActive(false);
                    }
                    else if(GameManager.instance.state == GameManager.State.Ramming)
                    {
                        GameManager.instance.state = GameManager.State.Idle;
